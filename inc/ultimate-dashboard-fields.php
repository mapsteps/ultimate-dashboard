...
function udb_main_metabox() {
    add_meta_box( 'ultimate-dashboard-main-metabox', __( 'Ultimate Dashboard', 'ultimatedashboard' ), 'udb_main_meta_callback', 'udb_widgets', 'normal', 'high' );
}
add_action( 'add_meta_boxes', 'udb_main_metabox' );

// Position Metabox
function udb_position_metabox() {
    add_meta_box( 'ultimate-dashboard-position-metabox', __( 'Position', 'ultimatedashboard' ), 'udb_position_meta_callback', 'udb_widgets', 'side' );
}
add_action( 'add_meta_boxes', 'udb_position_metabox' );

// Priority Metabox
function udb_priority_metabox() {
    add_meta_box( 'ultimate-dashboard-priority-metabox', __( 'Priority', 'ultimatedashboard' ), 'udb_priority_meta_callback', 'udb_widgets', 'side' );
}
...
function udb_position_meta_callback( $post ) {
	...
	<div>
		...
		<label for="udb-metabox-content-normal"><?php _e( 'Left column', 'ultimatedashboard' ); ?></label>
	</div>

	<div>
		<input id="udb-metabox-content-side" type="radio" name="udb_metabox_position" value="side" <?php checked( $udb_stored_meta, 'side' ); ?> />
		<label for="udb-metabox-content-side"><?php _e( 'Right column', 'ultimatedashboard' ); ?></label>
	</div>
...
function udb_main_meta_callback() { ?>
	...
	<?php
		...
		$nav_tabs = array(
			'<a class="nav-tab udb-icon-tab nav-tab-active" href="#">'. __( 'Icon Widget', 'ultimatedashboard' ) .'</a>',
		);
